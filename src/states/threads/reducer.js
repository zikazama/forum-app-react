/**
 * @TODO: Define the reducer for the talks state
 */
import { ActionType } from './action'

function threadsReducer (threads = [], action = {}) {
  switch (action.type) {
    case ActionType.RECEIVE_THREADS:
      return action.payload.threads
    case ActionType.ADD_THREAD:
      return [action.payload.thread, ...threads]
    case ActionType.UP_VOTE_THREAD:
      return threads.map((thread) => {
        if (thread.id === action.payload.id) {
          let upVotesBy = null
          let downVotesBy = null
          if (!thread.upVotesBy.includes(action.payload.userId) && !thread.downVotesBy.includes(action.payload.userId)) {
            upVotesBy = thread.upVotesBy.concat([action.payload.userId])
            downVotesBy = thread.downVotesBy
          } else if (thread.upVotesBy.includes(action.payload.userId) && !thread.downVotesBy.includes(action.payload.userId)) {
            upVotesBy = thread.upVotesBy.filter((id) => id !== action.payload.userId)
            downVotesBy = thread.downVotesBy
          } else if (!thread.upVotesBy.includes(action.payload.userId) && thread.downVotesBy.includes(action.payload.userId)) {
            upVotesBy = thread.upVotesBy.concat([action.payload.userId])
            downVotesBy = thread.downVotesBy.filter((id) => id !== action.payload.userId)
          }
          return {
            ...thread,
            upVotesBy,
            downVotesBy
          }
        }
        return thread
      })
    case ActionType.DOWN_VOTE_THREAD:
      return threads.map((thread) => {
        if (thread.id === action.payload.id) {
          let upVotesBy = null
          let downVotesBy = null
          if (!thread.upVotesBy.includes(action.payload.userId) && !thread.downVotesBy.includes(action.payload.userId)) {
            downVotesBy = thread.downVotesBy.concat([action.payload.userId])
            upVotesBy = thread.upVotesBy
          } else if (!thread.upVotesBy.includes(action.payload.userId) && thread.downVotesBy.includes(action.payload.userId)) {
            downVotesBy = thread.downVotesBy.filter((id) => id !== action.payload.userId)
            upVotesBy = thread.upVotesBy
          } else if (thread.upVotesBy.includes(action.payload.userId) && !thread.downVotesBy.includes(action.payload.userId)) {
            downVotesBy = thread.downVotesBy.concat([action.payload.userId])
            upVotesBy = thread.upVotesBy.filter((id) => id !== action.payload.userId)
          }
          return {
            ...thread,
            upVotesBy,
            downVotesBy
          }
        }
        return thread
      })
    case ActionType.NEUTRAL_VOTE_THREAD:
      return threads.map((thread) => {
        if (thread.id === action.payload.id) {
          let upVotesBy = null
          let downVotesBy = null
          if (thread.upVotesBy) {
            if (thread.upVotesBy.includes(action.payload.userId)) {
              upVotesBy = thread.upVotesBy.filter((id) => id !== action.payload.userId)
            } else {
              upVotesBy = []
            }
          }
          if (thread.downVotesBy) {
            if (thread.downVotesBy.includes(action.payload.userId)) {
              downVotesBy = thread.downVotesBy.filter((id) => id !== action.payload.userId)
            } else {
              downVotesBy = []
            }
          }
          return {
            ...thread,
            upVotesBy,
            downVotesBy
          }
        }
        return thread
      })
    default:
      return threads
  }
}

export default threadsReducer
